...
//2013.8.29
    ...
    Result operateAsyn(final Object instance, final String methodname, final Class[] argsType, final Object[] argsValue) {
        ...
        PoolExector.tpe().execute(new Runnable() {
            public void run() {
                try {
                    ...
                    if (obj != null)
                        rt.setResult(obj);
                    rt.setReady(Result.READY);
                } catch (Throwable e) {
                    LogUtil.info("operateAsyn", "exception", e);
                    rt.setReady(Result.EXCEPTION);
                }
            }
        });
        return rt;
