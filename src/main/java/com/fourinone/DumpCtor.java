...
class DumpCtor extends Contractor implements CoolHashBase, CoolHashClient {
    ...
    private ByteWriteParser bwp = DumpAdapter.getByteWriteParser();
    private ByteReadParser brp = DumpAdapter.getByteReadParser();
    ...
    private WareHouse[] dump(String[] k, Object[] v, Filter f, byte c, boolean p, String... psk) {
        ...
        for (Entry me : (Set<Entry>) wh.entrySet()) {
    ...
    private WareHouse getKeyMetaDump(CoolHashMap dump) {
        ...
        for (Entry me : (Set<Entry>) dump.getEntrySet()) {
            if (kl.size() == num) {
        ...
        for (WareHouse wr : whr) {
            for (Entry me : (Set<Entry>) wr.entrySet()) {
                String cd = (String) me.getKey();
    ...
    public Object remove(String key) throws CoolHashException {
        ...
        return bts != null ? ct.getTargetObject(bts, brp) : null;
    }
    ...
    public <T> T get(String key, Class<T> valueType, byte c, boolean point, String... pointSubKey) throws CoolHashException {
        chex.checking(key);
        byte[] bts = writeAhead.getMark() && writeAhead.containsKey(key) ? (byte[]) writeAhead.get(key) : dump(key, null, c, point, pointSubKey);
        return bts != null ? ct.getTargetObject(bts, valueType, brp) : null;
    }
    ...
    public <T> Object put(String key, T value) throws CoolHashException {
        ...
        return put(key, value, (byte) 0x14);
    }

    private <T> Object put(String key, T value, byte c) throws CoolHashException {
        return put(key, value, c, false);
    }

    //2017.10
    private <T> Object put(String key, T value, byte c, boolean point, String... subKey) throws CoolHashException {
        byte[] bts = value != null ? ct.getTargetBytes(value, bwp) : null;//getTargetBytes(value,bwp,code)
        chex.checking(key, bts);
        bts = writeAhead.getMark() ? (byte[]) writeAhead.put(key, bts) : dump(key, bts, c, false, subKey);
        return bts != null ? ct.getTargetObject(bts, brp) : null;
    }
    ...
    public String putPoint(String keyPoint, String key) throws CoolHashException {
        ...
        chex.checking(key);
        byte[] bts = writeAhead.getMark() ? (byte[]) writeAhead.put(keyPoint, ct.getBytes(key, bwp)) : dump(keyPoint, ct.getBytes(key, bwp), (byte) 0x14, false);
        return bts != null ? ct.getTargetObject(bts, String.class, brp) : null;
    }
	...
	/*public void exit(){
		ParkPatternExector.resetParkLocal();
		super.exit();
	}//2014.10.17*/
