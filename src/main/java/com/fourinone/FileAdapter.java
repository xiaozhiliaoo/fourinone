...
public class FileAdapter extends File {
    ...
    public static final short[] ConstBit = new short[]{0 << 1, 1 << 1, 1 << 2, 1 << 3, 3 << 1, 5 << 1, 6 << 1, 7 << 1, 1 << 0, 8 << 1};//2017.10
    ...
    private List<FileAdapter> closeList = new ArrayList<FileAdapter>();
    ...
    public void close() {
        try {
            ...
            closeBuffer(mbread);
            closeBuffer(mbwrite);
            for (FileAdapter faclose : closeList)
                faclose.close();
            closeList.clear();
        } catch (Exception e) {
    ...
    void close(FileAdapter fa) {
        if (!equals(fa)) {
            close();
        } else fa.closeList.add(this);
    }
