...
public class CoolHashMap extends LinkedHashMap implements Map, CoolHashBase {
    static boolean newParserFlag = true;
    private ByteWriteParser thebwp = null;
    private ByteReadParser thebrp = null;

    public CoolHashMap() {
        super();
        if (newParserFlag) {
            thebwp = DumpAdapter.getByteWriteParser();
            thebrp = DumpAdapter.getByteReadParser();
        }
    }
    ...
    private class DataMapEntry implements Entry, Comparable<Entry> {
        private String k;
        private Object v;
    ...
    public <T> Collection<T> values(Class<T> classType) {
        ...
        for (Iterator<byte[]> i = (Iterator<byte[]>) getValues().iterator(); i.hasNext(); )
            vls.add(ct.getTargetObject(i.next(), classType, thebrp));
        return vls;
    }
    ...
    public <T> T[] getValues(Class<T> classType) {
        ...
        int j = 0;
        for (Iterator<byte[]> i = (Iterator<byte[]>) getValues().iterator(); i.hasNext(); ) {
            vls[j++] = ct.getTargetObject(i.next(), classType, thebrp);
        }
        return vls;
    }
    ...
    public boolean containsValue(Object value) {
        return super.containsValue(ct.getTargetBytes(value, thebwp));
    }

    @Override
    public Set<Entry> entrySet() {
        Set<Entry> es = new LinkedHashSet<Entry>(size());
        Set<Entry> meset = getEntrySet();
        for (Entry me : meset)
            es.add(new DataMapEntry((String) me.getKey(), get(me.getKey())));
        return es;
    }

    Set<Entry> getEntrySet() {
        ...
        if (o instanceof CoolHashMap) {
            ...
            if (size() == odm.size()) {
                ...
                for (int i = 0; i < meset.length; i++) {
                    Entry me = (Entry) meset[i];
                    Entry odmme = (Entry) odmmeset[i];
    ...
    public Object get(Object key) {
        ...
        return vbs != null ? ct.getTargetObject(vbs, thebrp) : vbs;
    }
    ...
    public <T> T get(String key, Class<T> classType) {
        return ct.getTargetObject((byte[]) getValue(key), classType, thebrp);
    }
    ...
    public Object putPoint(String keyPoint, String key) {
        return chex.checkKey(key) ? putValue(keyPoint, ct.getBytes(key, thebwp)) : null;
    }

    //2017.10
	/*public Object putPlus(Object key, Object plusValue){
		Object value=chex.checkTypePlus(get(key), plusValue);
    ...
    private Object putValue(Object key, Object value, boolean t) {
        if (key != null && value != null) {
            value = t ? ct.getTargetBytes(value, thebwp) : value;//bytes[length-1]=putnx or putplus code;
    ...
    public void putAll(Map m) {
        if (m instanceof CoolHashMap)
            ...
            LogUtil.fail("[putAll]", "[IllegalArgumentException]", "add failed, the Map m is not instance of CoolHashMap!");
    }
    ...
    public Entry[] sort() {
        Entry[] arr = entrySet().toArray(new Entry[0]);
        Arrays.sort(arr);
        return arr;
    }

    public Entry[] sort(Comparator<Entry> comp) {
        Entry[] arr = entrySet().toArray(new Entry[0]);
        Arrays.sort(arr, comp);
        return arr;
    }
