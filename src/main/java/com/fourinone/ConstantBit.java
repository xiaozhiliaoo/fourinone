    ...
    enum Target implements ConstantBit, Filter.Condition {
        STRING(DumpAdapter.ConstBit[0], DumpAdapter.ConstBit[0]) {
            public boolean matchAction(Object key, Object value) {
                ...
                Filter.Condition cd = Action.valueOf(filter.getFilterKey());
        ...
        COOLBITSET(DumpAdapter.ConstBit[9], DumpAdapter.ConstBit[0]) {
            public byte[] getBytes(Object value, ByteWriteParser bwp) {
                if (bwp == null)
                    return getBytes(value);

                bwp.reset();
                //bwp.writeBytes(((CoolBitSet)value).bitArr);//not gzip 2017.10
                bwp.writeObject(((CoolBitSet) value).bitArr);
                bwp.writeBytes(new byte[]{(byte) codeKey});
                return bwp.getBytes();
				
				/*byte[] bts = STATGOBJECT.getBytes(value, bwp);
				bts[bts.length-1]=(byte)codeKey;
				return bts;*/
            }

            public CoolBitSet getObject(byte[] bts, ByteReadParser brp) {
				/*byte[] btsnew = new byte[bts.length-1];
				System.arraycopy(bts,0,btsnew,0,bts.length-1);
				return new CoolBitSet(btsnew);*/

                if (brp == null)
                    return (CoolBitSet) getObject(bts);

                brp.reset(bts);
                //byte[] btsnew = brp.read(bts.length-1);//not gzip 2017.10
                byte[] btsnew = (byte[]) brp.readObject(bts.length - 1);
                return new CoolBitSet(btsnew);

                /*return (CoolBitSet)STATGOBJECT.getObject(bts, brp);*/
            }
        };
        ...
        static <T> byte[] getTargetBytes(T value, ByteWriteParser bwp) {
            ConstantBit<T> cb = Target.getTarget(value);
        ...
        static <T> T getTargetObject(byte[] bts, Class<T> value, ByteReadParser brp) {
            if (bts == null || bts.length == 0) {
                ...
                ConstantBit<T> cb = value != null ? Target.getTarget(value) : Target.getTarget(bts[bts.length - 1]);
