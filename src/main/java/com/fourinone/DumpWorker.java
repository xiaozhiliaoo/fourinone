...
class DumpWorker extends MigrantWorker implements CoolHashBase {
    ...
    private Object dump(String d, String[] k, Object[] v, Filter f, byte c, boolean p, String... psk) {
        ...
        if (da.exists() && da.length() != 0) {
            ...
            if (cmp.size() > 0) {
                ...
                Set<Entry> meset = (Set<Entry>) wh.entrySet();
                for (Entry me : meset) {
                ...
                meset = (Set<Entry>) cmp.getEntrySet();
                for (Entry me : meset)
    ...
    private boolean checkMatch(String kn, String fw) {
		...
		*/
        ...
        if (fw.equals("\u002A"))//||fw.equals(kn)
            return true;
        if (fw.length() > kn.length())
            return false;
        StringTokenizer knst = new StringTokenizer(kn, "\u002E");
        StringTokenizer fwst = new StringTokenizer(fw, "\u002E");
        if (!fw.endsWith("\u002E\u002A") && fwst.countTokens() != knst.countTokens())
            return false;
        while (fwst.hasMoreTokens()) {
            String fwstr = fwst.nextToken();
            String knstr = knst.nextToken();
			/*if(!fwstr.equals("\u002A")&&!fwstr.equals(knstr))
				return false;*/
            //2015.7.28
            if (!fwstr.equals("\u002A")) {
                if (!fwstr.equals(knstr)) {
                    int at = fwstr.indexOf("\u002A");
                    if (at == -1 || fwstr.length() > knstr.length())
                        return false;
                    else if (at == 0 && !knstr.endsWith(fwstr.substring(at + 1)))
                        return false;
                    else if (at == fwstr.length() - 1 && !knstr.startsWith(fwstr.substring(0, at)))
                        return false;
                    else if (at > 0 && (!knstr.endsWith(fwstr.substring(at + 1)) || !knstr.startsWith(fwstr.substring(0, at))))
                        return false;
                }
            }
        }
    ...
    //2017.10
    byte[] dumpHandle(byte[] vl, byte c, boolean p, String... psk) {
        if (c == 0x2) {
            if (!p && psk[0] != null) {
                int i = 0;
                if (vl != null && vl[vl.length - 1] == (byte) DumpAdapter.ConstBit[9]) {
                    CoolBitSet cbs = (CoolBitSet) ct.getTargetObject(vl);
                    i = cbs.get(Integer.parseInt(psk[0])) ? 1 : 0;
                }
                vl = ct.getTargetBytes(i);
            }
        }
        return vl;
    }
    ...
    //2017.10
    byte[][] dumpHandle(byte[] vl, byte[] v, byte c, boolean p, String... psk) {
        byte[][] varr = new byte[2][];
        if (c == 0x18) {
            Object value = chex.getCheckPlus(ct.getTargetObject(vl), ct.getTargetObject(v));
            v = value != null ? ct.getTargetBytes(value) : vl;
        }
        if (c == 0x1c) {
            CoolBitSet cbs = null;
            if (vl != null && vl[vl.length - 1] == (byte) DumpAdapter.ConstBit[9])
                cbs = (CoolBitSet) ct.getTargetObject(vl);
            else
                cbs = new CoolBitSet();
            int i = (int) ct.getTargetObject(v);
            vl = ct.getTargetBytes(cbs.put(i));
            v = ct.getTargetBytes(cbs);
			
			/*if(vl!=null&&vl[vl.length-1]==(byte)DumpAdapter.ConstBit[9]){
				int i = (int)ct.getTargetObject(v);
				vl[i>>0x3]|=(byte)0x1<<(i&0x7);
				v = vl;
				vl = ct.getTargetBytes(1);
			}else{
				CoolBitSet cbs = new CoolBitSet();
				int i = (int)ct.getTargetObject(v);
				vl = ct.getTargetBytes(cbs.put(i));
				v = ct.getTargetBytes(cbs);
			}*/
        ...
        if (c == 0x1d) {
            if (!p && psk[0] == null) {
                CoolBitSet cbsv = (CoolBitSet) ct.getTargetObject(v);
                if (vl != null && vl[vl.length - 1] == (byte) DumpAdapter.ConstBit[9]) {
                    CoolBitSet cbsvl = (CoolBitSet) ct.getTargetObject(vl);
                    int n = cbsvl.set(cbsv);
                    vl = ct.getTargetBytes(n);
                    v = ct.getTargetBytes(cbsvl);
                } else
                    vl = ct.getTargetBytes(cbsv.getTotal());
            }
            if (!p && psk[0] != null) {
                if (vl != null && vl[vl.length - 1] == (byte) DumpAdapter.ConstBit[9]) {
                    CoolBitSet cbsv = (CoolBitSet) ct.getTargetObject(v);
                    CoolBitSet cbsvl = (CoolBitSet) ct.getTargetObject(vl);
                    cbsvl.binaryOperation(psk[0], cbsv);
                    v = ct.getTargetBytes(cbsvl);
                }
                vl = v;
            }
    ...
    public WareHouse doTask(WareHouse inhouse) {
        ...
        try {
            ...
            String[] psk = (String[]) inhouse.get(0x32);//2017.10
            if (c < 0x1e) {
            ...
            LogUtil.info("[DumpWorker]", "[doTask]", ex.toString());//ex.printStackTrace();
        }
        return wh;
    }

    void waitWorking(String[] args) {
        CoolHashMap.newParserFlag = false;
