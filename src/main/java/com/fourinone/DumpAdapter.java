...
class DumpAdapter extends FileAdapter {
    ...
    final static long ms = mk + m(ConfigContext.getREGIONLENGTH()) + ConstBit[8] + ConstBit[5];//mk+mv? or getRegion=max(mv,getConfig)
    final static long ml = m(ConfigContext.getLOADLENGTH());
    final static String src = ConfigContext.getDATAROOT();
    static boolean windows = BeanContext.getWindows(true);
    ...
    DumpAdapter getKeyMeta(String src, String k) {
        ...
        if (k != null && k.length() > 0) {
            ...
            while (tokenizer.hasMoreTokens()) {
                ...
                //fb.append(separator).append(ck.equals("\u002A")?ck:getMetaString(ck));//2015.7.28 indexOf!=-1?
                fb.append(separator).append(ck.indexOf("\u002A") != -1 ? "\u002A" : getMetaString(ck));
            }
    ...
    String[] getKeyWildArr(String k) {
        String[] ks = k.split("\\u002E");
        for (int i = 0; i < ks.length; i++)
            ks[i] = ks[i].indexOf("\u002A") != -1 ? "\u002A" : getMetaString(ks[i]); //ks[i].equals("\u002A")?ks[i]:getMetaString(ks[i]);//2015.7.28 indexOf()!=-1;
